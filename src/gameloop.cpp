...
GameSession::check_end_conditions()
{
	...
	if (!end_sequence && tux->base.x >= endpos) {
		...
		tux->invincible_timer.start(
		    7000); //FIXME: Implement a winning timer for the end sequence (with special winning animation etc.)
...
void GameSession::draw()
{
	...
	if (game_pause) {
		int x = screen->h / 20;
		for (int i = 0; i < x; ++i)
			fillrect(i % 2 ? (pause_menu_frame * i) % screen->w : -((
			             pause_menu_frame * i) % screen->w) , (i * 20 + pause_menu_frame) % screen->h,
			         screen->w, 10, 20, 20, 20, rand() % 20 + 1);
		fillrect(0, 0, screen->w, screen->h, rand() % 50, rand() % 50, rand() % 50,
		         128);
