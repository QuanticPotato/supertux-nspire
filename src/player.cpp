...
Player::handle_horizontal_input()
{
	...
	float dirsign = 0;
	if (input.left == DOWN && input.right == UP && (!duck
	        || physic.get_velocity_y() != 0)) {
		old_dir = dir;
		dir = LEFT;
		dirsign = -1;
	} else if (input.left == UP && input.right == DOWN && (!duck
	           || physic.get_velocity_y() != 0)) {
		old_dir = dir;
		dir = RIGHT;
		dirsign = 1;
...
Player::grabdistros()
{
	...
	if (!dying) {
		...
		if (size == BIG) {
			...
			World::current()->trygrabdistro(base.x + 31, base.y + base.height / 2,
			                                NO_BOUNCE);
		}
...
Player::check_bounds(bool back_scrolling, bool hor_autoscroll)
{
	...
	if (hor_autoscroll) {
		if (base.x == scroll_x)
			if ((issolid(base.x + 32, base.y) || (size != SMALL && !duck
			                                      && issolid(base.x + 32, base.y + 32))) && (dying == DYING_NOT))
				kill(KILL);
