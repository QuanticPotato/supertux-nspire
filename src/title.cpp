...
void generate_contrib_menu()
{
	...
	for (int i = 0; i < worldmap_list.num_items; i++) {
		...
		contrib_menu->additem(MN_ACTION, worldmap.get_world_title(), 0, 0,
		                      i + level_subsets.num_items);
	}
...
void title(void)
{
	...
	string_list_type files = dfiles("levels/worldmaps/", ".stwm",
	                                "couldn't list worldmaps");
	...
	while (Menu::current()) {
		...
		double frame_ratio = ((double)(update_time - last_update_time)) / ((
		                         double)FRAME_RATE);
		if (frame_ratio >
		        1.5) /* Quick hack to correct the unprecise CPU clocks a little bit. */
