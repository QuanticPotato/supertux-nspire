...
void
Surface::draw_part(float sx, float sy, float x, float y, float w, float h,
...
void
Surface::draw_stretched(float x, float y, int w, int h, Uint8 alpha,
                        bool update)
{
	if (impl) {
...
SurfaceOpenGL::SurfaceOpenGL(const std::string &file, int x, int y, int w,
                             int h, int use_alpha)
{
...
SurfaceOpenGL::create_gl(SDL_Surface *surf, GLuint *tex)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB10_A2, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
	             conv->pixels);
