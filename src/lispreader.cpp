		...
		case TOKEN_PATTERN_OPEN_PAREN : {
			...
			do {
				...
				if (car == &error_object || car == &end_marker) {
					...
					if (lisp_nil_p(last))
						obj = last = (token == TOKEN_OPEN_PAREN ? lisp_make_cons(car,
						              lisp_nil()) : lisp_make_pattern_cons(car, lisp_nil()));
					else
...
static int _match_pattern(lisp_object_t *pattern, lisp_object_t *obj,
                          lisp_object_t **vars);

static int
_match_pattern_var(lisp_object_t *pattern, lisp_object_t *obj,
                   lisp_object_t **vars)
{
...
int
lisp_match_pattern(lisp_object_t *pattern, lisp_object_t *obj,
                   lisp_object_t **vars, int num_subs)
{
	int i;
...
int
lisp_match_string(const char *pattern_string, lisp_object_t *obj,
                  lisp_object_t **vars)
{
	lisp_object_t *pattern;
	int result;
...
// This is a hook version of the original lisp_read_from_file.
// It actualy load a string, based on the filename
lisp_object_t *lisp_read_from_file(const std::string &filename)
{
	lisp_stream_t stream;
	const char *fileContent = getLevelData(filename);
	lisp_object_t *obj = 0;

	if(fileContent) {
		lisp_stream_init_string(&stream, (char*)fileContent);
		obj = lisp_read(&stream);
	} else {
		DEBUG_ERR2("Cannot load %s !", filename.c_str())
	}
