...
MenuItem *
MenuItem::create(MenuItemKind kind_, const char *text_, int init_toggle_,
                 Menu *target_menu_, int id, int *int_p_)
{
	...
	if (item.size() != 0) {
		switch (menuaction) {
			...
			case MENU_ACTION_INPUT:
				if (item[active_item].kind == MN_TEXTFIELD
				        || (item[active_item].kind == MN_NUMFIELD && mn_input_char >= '0'
				            && mn_input_char <= '9')) {
					if (item[active_item].input != NULL) {
						int i = strlen(item[active_item].input);
						item[active_item].input = (char *) realloc(item[active_item].input,
						                          sizeof(char) * (i + 2));
		...
		case MN_CONTROLFIELD: {
			...
			if (pitem.kind == MN_TEXTFIELD || pitem.kind == MN_NUMFIELD) {
				if (active_item == index)
					gold_text->draw_align((pitem.get_input_with_symbol(true)).c_str(),
					                      x_pos + text_pos, y_pos, A_HMIDDLE, A_VMIDDLE, 2);
				else
					gold_text->draw_align((pitem.get_input_with_symbol(false)).c_str(),
					                      x_pos + text_pos, y_pos, A_HMIDDLE, A_VMIDDLE, 2);
			} else
...
int Menu::get_width() const
{
	...
	int menu_width = 0;
	for (unsigned int i = 0; i < item.size(); ++i) {
		int w = strlen(item[i].text) + (item[i].input ? strlen(item[i].input) + 1 : 0) +
		        strlen(string_list_active(item[i].list));
		if (w > menu_width) {
			menu_width = w;
