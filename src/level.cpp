...
void LevelSubset::load(char *subset)
{
	...
	if (faccessible(filename)) {
		...
		if (faccessible(str)) {
			...
			snprintf(filename, 1024, "%s/images/status/level-subset-info.png",
			         datadir.c_str());
			delete image;
	...
	for (i = 1; i != -1; ++i) {
		...
		if (!faccessible(filename)) {
			snprintf(filename, 1024, "%s/levels/%s/level%d.stl", datadir.c_str(), subset,
			         i);
			if (!faccessible(filename))
				break;
		}
...
Level::load_gfx()
{
	if (!bkgd_image.empty()) {
		...
		if (!faccessible(fname))
			snprintf(fname, 1024, "%s/images/background/%s", datadir.c_str(),
			         bkgd_image.c_str());
		delete img_bkgd;
...
void Level::load_image(Surface **ptexture, string theme, const  char *file,
                       int use_alpha)
{
	char fname[1024];
	...
	if (!faccessible(fname))
		snprintf(fname, 1024, "%s/images/themes/%s/%s", datadir.c_str(), theme.c_str(),
		         file);
