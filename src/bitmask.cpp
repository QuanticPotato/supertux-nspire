...
bitmask *bitmask_create(int w, int h)
{
	...
	temp->w = w;
	temp->h = h;
	temp->bits = (long unsigned int *) calloc(h * ((w - 1) / BITW_LEN + 1),
	             sizeof(BITW));
	if (! temp->bits) {
		free(temp);
		return 0;
...
bitmask *bitmask_create_SDL(SDL_Surface *surf)
{
	...
	temp->w = surf->w;
	temp->h = surf->h;
	temp->bits = (long unsigned int *) calloc(surf->h * ((surf->w - 1) / BITW_LEN +
	             1), sizeof(BITW));
	if (! temp->bits) {
		free(temp);
		return 0;
...
int bitmask_overlap(const bitmask *a, const bitmask *b, int xoffset,
                    int yoffset)
{
	BITW *a_entry, *a_end;
	BITW *b_entry;
...
int bitmask_overlap_pos(const bitmask *a, const bitmask *b, int xoffset,
...
int bitmask_overlap_area(const bitmask *a, const bitmask *b, int xoffset,
                         int yoffset)
{
	BITW *a_entry, *a_end;
	BITW *b_entry;
